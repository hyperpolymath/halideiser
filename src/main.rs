...
//
// halideiser CLI — Accelerate image/video pipelines via Halide
...
/// halideiser — Accelerate image/video pipelines via Halide
#[derive(Parser)]
...
#[derive(Subcommand)]
enum Commands {
    /// Initialise a new halideiser.toml manifest in the current directory.
    Init {
        #[arg(short, long, default_value = ".")]
        path: String,
    },
    /// Validate a halideiser.toml manifest.
    Validate {
        #[arg(short, long, default_value = "halideiser.toml")]
        manifest: String,
    },
    /// Generate Halide wrapper, Zig FFI bridge, and C headers from the manifest.
    Generate {
        #[arg(short, long, default_value = "halideiser.toml")]
        manifest: String,
