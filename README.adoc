...
[arabic]
. *Describe* your pipeline in `+halideiser.toml+` — stages, buffer
dimensions, data types, target hardware
. *Validate* — the Idris2 ABI layer formally proves buffer bounds,
dimension compatibility, and stage connectivity
. *Generate* — Halide algorithm code is emitted with `+Func+`, `+Var+`,
and scheduling primitives
. *Tune* — the auto-tuner searches the schedule space (tile sizes, loop
orders, parallelism) for optimal performance
. *Compile* — the tuned schedule is compiled to native code via LLVM
...
** `+tile(x,+` `+y,+` `+xi,+` `+yi,+` `+tx,+` `+ty)+` — break loops into
tiles for cache locality
...
* *10–100x faster* image and video processing without writing Halide by
hand
